  ...
  async execute(interaction, client) {
    try {
      ...
      const shopEmbed = new EmbedBuilder()
        .setColor("#00ff00")
        .setTitle("Shop")
        .setDescription("Welcome to the shop! Browse items available for purchase:")
        
      for (let i = index; i < end; i++) {
        const item = shopItems[i];
        shopEmbed.addFields(
          { name: `Item: ${item.item}`, value: `Price: ${item.price} Obol ◈`, inline: true }
        )
      }
      ...
      await interaction.reply({ embeds: [shopEmbed], components: [actionRow] });
    } catch (error) {
      console.error("Error reading shop data:", error);
      await interaction.reply("An error occurred while fetching shop data. Please try again later.");
    }
  },
};
